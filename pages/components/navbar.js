...
export default function Navbar({ theme = "dark", onThemeToggle }) {
  ...
  const navItems = [{ name: "Login", href: "/login", icon: LogIn }]

  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-50 h-[24px] transition-all duration-500 ${
    ...
    >
      ...
      >
        <div className="max-w-7xl mx-auto px-6 py-4">
              ...
              {navItems.map((item) => {
                ...
                return (
                  <Link key={item.name} href={item.href}>
                    <Button
                      variant="ghost"
                      className={`relative px-8 py-3 rounded-xl font-medium transition-all duration-300 hover:scale-105 ${
                        theme === "dark"
          ...
          >
            <div className={`border-t pt-6 space-y-3 ${theme === "dark" ? "border-white/10" : "border-indigo-200/50"}`}>
              {navItems.map((item) => {
                const Icon = item.icon
                return (
