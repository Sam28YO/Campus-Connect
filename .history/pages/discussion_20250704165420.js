...
const Discussions = () => {
  ...
  const downvotePost = async (postId) => {
    try {
      ...
      setDiscussions((prev) =>
        prev.map((post) =>
          post.id === postId ? { ...post, downvotes, upvotes } : post
        )
      );
  ...
  const handleVote = async (discussionId, voteType) => {
    ...
    if (currentVote === voteType) {
      // Remove vote
      if (voteType === "up") {
        // Remove upvote logic would go here
      } else {
  ...
  const handleViewFullPost = async (discussion) => {
    ...
    await getPostComments(discussion.id);
  };
      ...
      >
        <div
          className={`absolute inset-0 bg-gradient-to-r ${
            community.color || "from-purple-500 to-indigo-500"
          ...
          <div className="flex items-center gap-4">
            <div
              className={`w-16 h-16 rounded-2xl bg-gradient-to-r ${
                community.color || "from-purple-500 to-indigo-500"
              } flex items-center justify-center shadow-lg group-hover:scale-110 group-hover:rotate-6 transition-all duration-300`}
            >
            ...
            <div>
              ...
              {typeof community.members !== "undefined" && (
                <p
                  className={`text-sm ${
                    theme === "dark" ? "text-gray-400" : "text-indigo-600"
                  }`}
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
          ...
          <div className="text-center">
            ...
            >
              {Math.floor((community.totalVotes || 0) / 100)}k
            </div>
            <div
              className={`text-xs ${
                theme === "dark" ? "text-gray-400" : "text-indigo-600"
      ...
      >
        <div className="flex items-start justify-between mb-6 relative z-10">
          <div className="flex items-start gap-4 flex-1">
            <div className="relative">
              ...
              >
                {discussion.user?.avatar || discussion.author?.charAt(0) || "U"}
              </div>
              {discussion.user?.isOnline && (
                <div className="absolute -bottom-1 -right-1 w-5 h-5 bg-green-400 rounded-full border-2 border-white animate-pulse"></div>
              )}
            ...
            <div className="flex-1">
              <div className="flex items-center gap-3 mb-2">
                ...
                >
                  {discussion.user?.name || discussion.author || "Anonymous"}
                </h4>
                <span
                  className={`text-sm ${
                    theme === "dark" ? "text-gray-400" : "text-gray-600"
                ...
                >
                  • {discussion.timestamp || discussion.createdAt || "Recently"}
                </span>
              </div>
            </div>
          </div>
        ...
        {comments[discussion.id] && comments[discussion.id].length > 0 && (
          <div className="mt-6 space-y-4">
            ...
            >
              Comments ({comments[discussion.id].length})
            </h4>
            {comments[discussion.id].map((comment) => (
              ...
              >
                <div className="flex items-start gap-3">
                  ...
                  >
                    {comment.user?.avatar || comment.author?.charAt(0) || "U"}
                  </div>
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      <span
                      ...
                      >
                        {comment.timestamp || comment.createdAt || "Recently"}
                      </span>
                    </div>
                    <p
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <button
                        onClick={() =>
                          handleDeleteComment(comment.id, discussion.id)
                        }
                        className={`text-xs font-medium transition-colors ${
                          theme === "dark"
      ...
      <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
        ...
        >
          ...
          <div className="p-6">
            ...
            >
              <div className="whitespace-pre-wrap">
                {selectedDiscussion.fullContent || selectedDiscussion.content}
              </div>
            </div>
